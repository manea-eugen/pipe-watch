...
- **Native macOS notifications** -- alerts on pipeline success, failure, or cancellation
...
- **Status-aware menu bar icon** -- icon changes color based on the worst active pipeline status (red = failed, blue = running, orange = pending, green = passed)
...
2. **`GitLabService`** (an Swift actor for thread safety) makes three sets of API calls:
   - `GET /api/v4/user` -- identifies the current user (cached after the first call)
   - `GET /api/v4/projects?membership=true&last_activity_after=24h_ago` -- finds recently active projects
   - `GET /api/v4/projects/{id}/pipelines?username={you}&updated_after=24h_ago` -- fetches your pipelines per project, concurrently
3. **State diffing** -- the monitor compares each pipeline's current status against its previously known status. If a pipeline transitioned to a terminal state (`success`, `failed`, `canceled`), it triggers a notification.
...
├── PipelineNotifications.entitlements   # Network + Keychain permissions
├── Assets.xcassets/                     # App icon assets
│
└── PipelineNotifications/               # Source code
    ├── PipelineNotificationsApp.swift   # @main entry, MenuBarExtra + AppDelegate
    │
    ├── Models/
    │   ├── Pipeline.swift               # Pipeline, PipelineStatus, TrackedPipeline
    ...
    │
    ├── Views/
    │   ├── PipelineListView.swift       # Main popover: pipeline list grouped by project
    │   ├── PipelineRowView.swift        # Single pipeline row (status, branch, duration)
...
3. Give it a name (e.g. "Pipeline Notifications")
